...
import { ControllersService } from "@/client";
import {
  DialogActionTrigger,
  DialogBody,
  DialogCloseTrigger,
...
const DeleteController = ({ id }: { id: string }) => {
  ...
  const deleteController = async (id: string) => {
    await ControllersService.deleteController({ greenhouseId: id, controllerId: id });
  };
  ...
  const mutation = useMutation({
    ...
    onError: () => {
      showErrorToast("An error occurred while deleting the controller.");
    },
    onSettled: () => {
