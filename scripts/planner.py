...
from ai_config import TEMPLATES_DIR, ai
    ...
    if trigger_file.exists():
        try:
            ...
            if age < 900:  # 15 min
                ...
                reasons = [
                    f"{d['param']} deviation (observed {d['observed']} vs forecast {d['forecasted']})"
        ...
        if len(parts) >= 15 and parts[0].strip().startswith(("Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun")):
            ...
            def cap_horizon(t):
                ...
                if m < 0:
                    return t
                ts = datetime(
                    row_date.year, row_date.month, row_date.day, m // 60, m % 60, tzinfo=timezone(timedelta(hours=-6))
                )
...
async def write_plan_to_db(plan: dict, greenhouse_id: str) -> dict:
    ...
    stats = {"waypoints": 0, "journal": False, "validation": False, "lesson": False, "filtered": 0}
        ...
        async with conn.transaction():
            ...
            if "transitions" in plan:
                ...
                for t in plan["transitions"]:
                    ...
                    for param, value in t.get("params", {}).items():
                        waypoints.append({"ts": ts, "parameter": param, "value": value, "reason": reason})
            else:
            ...
            if waypoints:
                ...
                if first_ts > now_ts:
                    ...
                    log.info("Added %d immediate-coverage waypoints at %s", len(now_wps), now_ts[:16])
            if filtered:
                log.warning(
                    "Filtered %d band-driven waypoints: %s", len(filtered), {wp["parameter"] for wp in filtered}
                )
            for wp in waypoints:
                await conn.execute(
                    """
            ...
            if prev and prev.get("plan_id") and prev.get("score"):
                ...
                if lesson_text:
                    existing = await conn.fetchval(
                        "SELECT id FROM planner_lessons WHERE lesson = $1 AND is_active = true LIMIT 1", lesson_text
                    )
                    if existing:
                        await conn.execute(
                            """
...
def main():
    ...
    log.info("Context assembled: %d chars in %.1fs (~%d tokens)", len(prompt), context_time, len(prompt) // 4)
    ...
    if provider == "anthropic":
        ...
        log.info("Tokens: input=%s output=%s", response.usage.input_tokens, response.usage.output_tokens)
    else:
    ...
    log.info(
        "Plan %s: %d transitions, hypothesis: %s",
        plan.get("plan_id", "?"),
        len(transitions),
    ...
    log.info(
        "DB writes: %d waypoints (%d band-driven filtered), journal=%s, validation=%s, lesson=%s",
        stats["waypoints"],
        stats["filtered"],
        stats["journal"],
        stats["validation"],
    ...
    log.info("Planner complete (%.1fs context + %.1fs inference + DB write)", context_time, elapsed)
