...
ACTION_PATTERNS = [
    ...
    (
        r"\bwilt(?:ed|ing|s)?\b|\byellow(?:ing|ed)?\b|\bspot(?:s|ted)?\b|\bbug(?:s)?\b|\bpest\b|\bmold(?:y)?\b|\brot(?:ting|ten)?\b",
        "observed",
    ),
    ...
    if result["action"] in ("planted", "moved") and not result["position"]:
        ...
        if result["crop"] or result["position"]:
            ...
            result["needs_clarification"] = (
                "I couldn't understand that. Try: 'Planted basil in HYDRO-5' or 'Strawberries in HYDRO-3 look wilted'"
            )
...
async def execute(parsed: dict, operator: str, image_url: str | None) -> dict:
    ...
    try:
        ...
        if action == "planted":
            ...
            response["message"] = (
                f"Got it — {parsed['crop']} planted in {parsed['position']}, stage: {parsed['stage'] or 'seedling'}"
            )
            ...
            if crop:
                ...
                """,
                    crop["id"],
                    operator,
                    f"Moved from {old_pos} to {parsed['position']}",
                )
            ...
            if parsed["crop"]:
                ...
                row = await conn.fetchrow("SELECT id FROM crops WHERE position = $1 AND is_active", parsed["position"])
                crop_id = row["id"] if row else None
