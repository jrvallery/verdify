...
# bundle.sh — Enhanced file bundling tool for code review and sharing
# Version: 2.0
# Author: Enhanced for home-infra project
# Last Modified: August 17, 2025
#
# DESCRIPTION:
#   Merges multiple text files into a single structured bundle file for easy sharing,
...
usage() {
  cat <<'USAGE'
Usage: bundle.sh [-p PATH] [-f FILE_LIST] [-o OUTPUT] [--progress] [--no-git] [--max-bytes N] [--ext-exclude EXT ...]
...
OPTIONS:
  ...
  -o, --output FILE        Output bundle file (default: project_bundle_YYYYMMDD_HHMMSS.txt)
...
EXAMPLES:
  ...
  ./bundle.sh -p backend/app --progress

  # Bundle from explicit file list (recommended for targeted bundling)
  ./bundle.sh -f my_project_files.txt -o project_bundle.txt

  # Bundle with size limits and exclusions
  ./bundle.sh -p src --max-bytes 1000000 --ext-exclude .min.js --ext-exclude .map

  # Force find-based scanning (no git)
  ./bundle.sh -p legacy_code --no-git --progress
...
# Defaults
PATH_ROOT="."
OUTPUT_FILE="project_bundle_$(date +%Y%m%d_%H%M%S).txt"
USE_GIT=1
SHOW_PROGRESS=0
MAX_BYTES=0
EXCLUDE_EXTS=()
...
# Output header
{
  ...
  echo "Generated by: bundle.sh v2.0 (enhanced with file list support)"
  echo
} > "$OUTPUT_FILE"
...
TMP_LIST="$(mktemp -t bundle_list.XXXXXX)"
...
echo "bundle complete: $OUTPUT_FILE (${files_included} files included, ${files_skipped} skipped)"
