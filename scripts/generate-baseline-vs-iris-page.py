...
def render(periods: list[Period], daily: list[Daily]) -> str:
    ...
    rows = [
        ...
        ("Energy/day", f"{baseline.kwh} kWh", f"{current.kwh} kWh", fmt_reduction(current.kwh, baseline.kwh, " kWh")),
        (
            "Cost/day",
            f"USD {baseline.cost_usd}",
            f"USD {current.cost_usd}",
            fmt_reduction(current.cost_usd, baseline.cost_usd, " USD"),
        ),
...
This is an operational comparison, not a controlled academic A/B test. The baseline window is the April 22-25, 2026 planner-offline run already documented in the public outage story. The current window is the following April 26-May 2, 2026 period, when normal Iris planning resumed.
...
<div class=\"data-table\">
  ...
  <div class=\"data-row\"><strong>Stress hours/day</strong><span>Heat + cold + VPD-high + VPD-low</span><p>Summed daily stress duration from corrected daily summary fields. This is not capped at one stress type; a hot-dry hour can count on more than one axis.</p></div>
  ...
  <div class=\"data-row\"><strong>Cost/day</strong><span>Electric + gas + water</span><p>Resource spend from measured or estimated daily summary fields. The greenhouse is solar-aligned but still uses grid electricity and gas heat.</p></div>
</div>
