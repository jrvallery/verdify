...
function clarifyText(value: string): { value: string; changed: boolean; alreadyClear: boolean } {
  const match = IRIS_RE.exec(value)
  if (!match) return { value, changed: false, alreadyClear: false }
  if (isAlreadyClear(value, match.index)) return { value, changed: false, alreadyClear: true }
      ...
      return [
        () => {
          return (tree: Root) => {
            ...
            function visit(node: HastNode, skipped = false, insideParagraph = false): void {
              if (done) return

              if (node.type === "text") {
                if (!skipped && insideParagraph && typeof node.value === "string" && IRIS_RE.test(node.value)) {
              ...
              const isSkipped =
                skipped ||
                (node.type === "element" && typeof node.tagName === "string" && SKIP_TAGS.has(node.tagName)) ||
                nodeClasses.some((className) => SKIP_CLASSES.has(className)) ||
                node.properties?.ariaHidden === true ||
                node.properties?.ariaHidden === "true" ||
                node.properties?.["aria-hidden"] === true ||
                node.properties?.["aria-hidden"] === "true"
              const isParagraph =
                insideParagraph || (node.type === "element" && typeof node.tagName === "string" && node.tagName === "p")
