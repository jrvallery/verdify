-- Migration 024: Staleness detector view
-- Joins sensor_registry with actual data to flag stale sensors
-- Uses UNION approach: one subquery per source_table pattern

CREATE OR REPLACE VIEW v_sensor_staleness AS
WITH last_seen_data AS (

  -- Climate sensors: each has a specific source_column
  -- Find the max ts where that column IS NOT NULL
  SELECT
    sr.sensor_id,
  ...
  -- Equipment state sensors: keyed by equipment name (strip 'equipment.' prefix from sensor_id)
  SELECT
    sr.sensor_id,
  ...
  -- System state sensors: keyed by entity name (strip 'state.' prefix)
  SELECT
    sr.sensor_id,
  ...
  -- Diagnostics sensors: keyed by column name (strip 'diag.' prefix)
  SELECT
    sr.sensor_id,
  ...
  -- Diagnostics text sensors (probe_health, reset_reason): use global MAX(ts)
  SELECT
    sr.sensor_id,
    (SELECT MAX(d.ts) FROM diagnostics d
