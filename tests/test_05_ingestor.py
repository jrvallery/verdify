...
class TestIngestorService:
    ...
    def test_no_recent_crashes(self):
        ...
        result = subprocess.run(
            ["journalctl", "-u", "verdify-ingestor", "--since", "1 hour ago",
             "--no-pager", "-q", "-o", "cat"],
...
class TestESP32Connection:
    ...
    def test_zone_vpd_data(self):
        ...
        for zone in ["vpd_south", "vpd_west", "vpd_east"]:
            val = db_query(
                f"SELECT {zone} FROM climate WHERE {zone} IS NOT NULL ORDER BY ts DESC LIMIT 1"
            )
    ...
    def test_equipment_state_tracked(self):
        ...
        count = db_query(
            "SELECT count(DISTINCT equipment) FROM equipment_state WHERE ts > now() - interval '1 hour'"
        )
...
class TestIngestorTasks:
    ...
    def test_setpoint_dispatcher_recent(self):
        ...
        age = db_query(
            "SELECT extract(epoch FROM now() - max(ts))::int FROM setpoint_changes WHERE source != 'esp32'"
        )
...
class TestDataIntegrity:
    ...
    def test_no_null_temp_in_recent_climate(self):
        ...
        nulls = db_query(
            "SELECT count(*) FROM climate WHERE ts > now() - interval '1 hour' AND temp_avg IS NULL"
        )
    ...
    def test_daily_summary_stress_consistent(self):
        ...
        for col in ["stress_hours_heat", "stress_hours_cold", "stress_hours_vpd_high", "stress_hours_vpd_low"]:
            val = db_query(
                f"SELECT max({col}) FROM daily_summary WHERE date >= CURRENT_DATE - 7"
            )
            if val:
